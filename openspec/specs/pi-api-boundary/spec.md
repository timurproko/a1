...
#### Scenario: A published version is skipped
- **WHEN** the proposal pull request for the newest published version was closed with the skip label
- **THEN** the sync SHALL propose the newest unskipped version newer than the pin, or none, and SHALL name the skipped versions in its output
- **AND** a proposal closed without the label SHALL be proposed again, and a manual dispatch naming the skipped version SHALL propose it
