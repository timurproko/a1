...
- Shell dialogs such as the model selector are built in `shell-selectors-dialogs.ts` on the public `#pi-tui` boundary and presented as the shell's replacement input (`root.setInputSurface`), the host the modal inventory records for the `models.*` family; the viewport keeps exposed transcript content interactive under every modal.
...
The selected autocomplete row is read from the editor's public list accessor (`getSelectedItem`) through the same reflective read the owned selection layer already uses for pinned editor state, never by patching a prototype; the shell supplies the tunnel command names, so the owned editor holds no skills knowledge of its own.
