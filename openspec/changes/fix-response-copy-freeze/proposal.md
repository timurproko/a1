...
- Preserve the full pasted URL and its chip while bounding explicit terminal hyperlink metadata to 64 KiB per editor render. Omit over-budget OSC 8 metadata before encoding/presentation rather than truncate the URL, reject a supported paste, or emit a partial target; ordinary in-budget links remain unchanged.
- Compact successfully classified path lists whose individual-chip presentation exceeds a 4,096 UTF-16-unit per-paste budget into one existing text-paste chip. Prepare its exact expanded paths off-thread, including repetitions and order, without adding separators, truncating content, or changing in-budget file/folder/image-file chips. This approved presentation exception retains ordinary paste-chip atomic editing and recovery.
- Preserve paste content, chip classification (subject to the approved oversized-path-list presentation exception), selection replacement, undo/redo, and input ownership while bounding concurrent requests and recovery. Distinct accepted pastes retain their order and reserved insertion positions; copy's newest-pending policy must not drop paste actions.
...
The existing `keep-streaming-ui-responsive-during-selection` plan owns broad selection/scrolling/render preparation optimization. This change owns response copy and ordinary-prompt paste through their clipboard/preparation/presentation boundaries, coordinates shared code against the implementation base, and does not duplicate that general renderer work. Prompt copy/cut payload semantics, `/copy`, Ctrl+C without a transcript selection, modal ownership, image safety, and `a1 pi` remain compatibility controls. Keep the existing change name for continuity with specification PR #359; the added paste scope is intentional. The initial and standalone expansion revisions were planning-only. With implementation PR #385 open, the user approved this URL-metadata specification/implementation refinement in the same PR. A synthetic near-16 MiB URL exposed oversized OSC 8 presentation and roughly 300 ms of application delay; this is not attribution of either reported physical freeze or acceptance of the unfinished implementation. The user subsequently approved the compact-path-list spec/code refinement in the same PR after 6,000 long-named paths generated over one million editor characters and a roughly 1.39-second synthetic application timer gap. Approval of that refinement is not physical acceptance or merge authorization.
