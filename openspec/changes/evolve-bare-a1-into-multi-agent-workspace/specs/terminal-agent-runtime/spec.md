...
A transparent session SHALL advertise no A1-authoritative resident surface, internal pane, virtual scrollback, replayable display stream, or visual reconnection. A1 SHALL NOT claim those capabilities from a shadow parser or lifecycle metadata. Transparent direct attachment SHALL remain an explicit single-foreground fallback and comparison baseline and SHALL NOT be routed through the composed-terminal implementation.
...
- **THEN** A1 SHALL use the separately declared composed-terminal capability and SHALL NOT add interception to transparent mode
...
- **THEN** the child and physical terminal SHALL retain native rendering and input authority without traversing or initializing the composed terminal host, pseudoterminal, parser, model, renderer, topology, or input router

#### Scenario: Composed host is unavailable or fails
- **WHEN** the composed terminal host is unsupported, fails its proof gate, or exits abnormally
