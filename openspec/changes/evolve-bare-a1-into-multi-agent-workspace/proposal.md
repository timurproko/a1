...
The reopened vanilla-parity prerequisite is closed: `repair-owned-pi-parity` was accepted and archived, and its provider-authentication requirement now lives in the `owned-pi-ui-foundation` spec. The first A1-specific Pi experience is delivered next by a sequence of custom-experience milestones — the settings screen, then the agent viewport, the input prompt, paste, and history — which together replace the previously planned single `customize-owned-pi-experience` change. This plan resumes only once that bare-minimum experience is complete, not when any one milestone lands. Bare `a1` runs that custom experience; `a1 pi` and `a1 sandbox` remain untouched vanilla Pi, altered only by Pi extensions. No milestone completion automatically resumes this held multi-agent plan.
