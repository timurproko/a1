...
>
> **Structured-tab scope superseded 2026-09-24.** On 2026-09-24 the user authorized multi-agent structured tabs with resident background agents. That work is now planned in `add-persistent-multi-agent-tabs`, which supersedes this change's structured-tab, workspace-identity, and reconnection requirements (`multi-agent-workspace`, `structured-agent-runtime`, and the structured parts of the `a1-shell` delta). Only the composed-terminal scope (arbitrary CLI panes, PTYs, split layouts, the terminal-host proof, and multiplexer integration) remains on hold here. It stays on hold until the user explicitly resumes it, and it should then build on the resident agent host rather than on a separate daemon.
