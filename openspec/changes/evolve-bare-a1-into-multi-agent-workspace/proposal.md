...
- Complete the separately planned `harden-pi-api-boundary` change before structured Pi tabs. Workspace and feature code SHALL consume vendor-neutral A1 ports; Pi selection, public-API compatibility checks, resource access, UI integration, and exact-version launch SHALL remain confined to an A1 composition root and bounded Pi integration.
- After fullscreen base-UX and Pi-boundary acceptance, add A1-owned tabs for multiple structured SDK-backed agents. These tabs switch semantic agent views and SHALL NOT initialize the terminal host, create PTYs, or imply arbitrary terminal-pane support.
