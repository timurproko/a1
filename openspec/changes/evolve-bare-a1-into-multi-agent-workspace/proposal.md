...
User-controlled comparison has reopened the claimed vanilla-parity prerequisite: A1 can expose models from persisted A1 credentials while `/login` reports the same providers as unconfigured. Multi-agent work is therefore paused while `repair-owned-pi-parity` restores a trustworthy single-agent baseline and a subsequent `customize-owned-pi-experience` change establishes the first accepted A1-specific Pi experience.
