...
- After the proof verdict, remove the fixed multipane presentation, restore the single fullscreen terminal path, and publish only an uncertified foundation `-dev.N` preview under npm `next`, with composed multipane behavior disabled and no composed-terminal support claim.
- Gate production tabs, layouts, and multi-agent presentation on a separately planned AddOne-owned Pi UI foundation. That foundation uses Pi's documented public SDK as the agent engine behind an AddOne-owned fullscreen interface; it does not patch, inspect, or deep-import Pi interactive-TUI internals. It reaches vanilla-first parity by adapting documented public Pi UI components where suitable and provenance-recorded MIT-licensed ports where ownership is needed, while exact upstream Pi remains available through `a1 pi`.
...
The change affects CLI launch routing, workspace/application state, supervision and storage, structured-agent adapters, a versioned local terminal-host protocol, pinned terminal-core/PTY source integration, terminal process ownership, in-terminal rendering/input, test infrastructure, platform-specific console executable packaging, documentation, and cross-platform release policy. Implementation requires a milestone branch, but that branch SHALL NOT merge into `develop` until the in-terminal proof gate passes. After that gate, the foundation preview may merge and publish only after the proof presentation has been removed and fullscreen-only behavior restored. Structured-agent work remains independently deliverable, while production tabs and multi-agent presentation remain blocked on the separate owned-UI foundation. The accepted meanings and transparent execution paths of `a1 pi` and `a1 sandbox` must not change.
