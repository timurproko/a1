...
>
> **Tab scope superseded 2026-09-24.** On 2026-09-24 the user authorized multi-agent tabs with resident background sessions. The user also decided that every tab is a terminal session, so that extensions stay native and arbitrary CLI tabs can follow on the same system. That work is planned in `add-persistent-multi-agent-tabs`. It supersedes this change's structured-tab and structured-runtime scope (`multi-agent-workspace`, `structured-agent-runtime`), which is no longer planned. It also supersedes the single-pane composed-terminal scope: the native terminal host, PTY sessions, reconnection, and certification. Its per-platform certification replaces the 2×2 isolated-worker proof gate for single-pane tabs. Only split layouts and multiplexer presentation remain on hold here until the user explicitly resumes them, and they should build on the resident terminal host.
