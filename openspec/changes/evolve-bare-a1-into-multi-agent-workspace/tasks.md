...
- [x] 5.1 Build a minimal A1-owned console terminal-host executable from pinned terminal-core, PTY, and host-input components that opens one fullscreen alternate-screen surface inside the current terminal without creating a desktop window and owns the event, rendering, and lifecycle loops; pass a non-interactive build, version/provenance inspection, and process start/clean-shutdown harness on an isolated worker.
...
- [x] 5.4 Complete and accept the separately specified `build-owned-pi-ui-foundation` change: use the public Pi SDK engine behind an A1-owned fullscreen root, reach vanilla-first base-UX parity for one session by adapting public components or provenance-recorded MIT ports, confine Pi types to adapters, forbid stock-`InteractiveMode` mutation and private/deep/hash coupling, and pass Pi-upgrade conformance.
- [x] 5.5 After the separately specified `harden-pi-api-boundary` change is complete and accepted, integrate the existing structured workspace into A1-owned tabs for multiple SDK-backed Pi agents through vendor-neutral A1 ports and composition-root wiring; pass independent transcript/tool/editor state, switching and input-target races, background activity/attention, lifecycle, failure isolation, restart/recovery, accessibility, bounded-resource, and architecture tests proving workspace and feature modules import neither Pi-specific contracts nor composed terminal-host code and do not launch or connect to the terminal host.
- [x] 5.6 Run fullscreen and structured-tab base-UX acceptance with the terminal multiplexer unavailable, then cut bare `a1` over to that structured workspace only: compare the vanilla-style preset with `a1 pi`, verify customization slots, recovery, the sole command, and explicit modes, and record that structured tabs create no PTYs and make no composed-terminal support claim.
- [x] 5.7 Merge the independently accepted owned-UI and structured-tab slice through clean `develop` and publish a unique `-dev.N` package under npm `next` with composed multipane behavior disabled, `latest` unchanged, no composed support claim, and registry/package-integrity verification.
...
- [ ] 5.9 Obtain a user-controlled manual verdict or attested disposable-worker physical verdict for the exact artifact; never launch, focus, drive, resize, or close terminal applications on the active workstation through automation.
- [ ] 5.10 Apply the stop/go gate: if every mandatory result is accepted, record permission to begin production multiplexer integration; otherwise record the failed evidence, stop composed-terminal work without custom rendering/input remediation or desktop-app fallback, and keep composed work unmergeable without weakening criteria. The published fullscreen UI and structured tabs remain independently usable.
- [ ] 5.11 After recording the verdict, preserve the exact proof evidence and remove the fixed 2×2 geometry, four-Pi default, dashed pane chrome, and proof-only multipane launch path from the shipping tree; restore the one-session fullscreen terminal host and pass fullscreen input/render/resize/cleanup plus explicit-mode regressions.
...
- [ ] 6.6 Package and verify the Windows console terminal host as an integrity-bound platform artifact selected by A1 without loading PTY or terminal-emulation code into Node; pass install, missing-host, tampered-host, incompatible-host, update, rollback, and package-content gates.
...
- [ ] 7.4 Build the composed workspace presentation through the accepted A1-owned UI composition contracts over authoritative host snapshots without painting terminal cells in Node; pass deterministic topology/status rendering, accessibility semantics, customization-slot isolation, and unavailable-capability behavior.
...
- [ ] 9.1 Derive a macOS A1 console terminal host from the pinned terminal-core and Unix PTY components while preserving the platform-neutral protocol and hot-path invariant; pass isolated build, topology, PTY, input, rendering, lifecycle, and package gates without using AppKit/Metal.
- [ ] 9.2 Derive a Linux A1 console terminal host from the pinned terminal-core and Unix PTY components while preserving the platform-neutral protocol and hot-path invariant; pass isolated build, topology, PTY, input, rendering, lifecycle, and package gates without using GTK.
...
- [ ] 9.4 Verify protocol conformance and observable workspace semantics across Windows, macOS, and Linux while permitting platform PTY/input implementations; reject platform-specific semantic drift at the A1 protocol boundary and document that desktop-native application shells remain postponed follow-up work.
