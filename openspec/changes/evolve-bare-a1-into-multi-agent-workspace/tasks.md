...
- [ ] 5.7 Merge the independently accepted owned-UI and structured-tab slice through clean `develop` and publish a unique `-dev.N` package under npm `next` with composed multipane behavior disabled, `latest` unchanged, no composed support claim, and registry/package-integrity verification.
