...
- Development validation run 35735212463 exposed a placement fixture that still required byte-identical selected-row ANSI. The repaired fixture preserves byte parity for every unselected row while comparing selected-row text and layout around the declared role difference; the placement, shell-component, and skills-tunnel suites pass all 60 focused tests.
- Development validation run 35736318981 passed all product scopes and exposed only a documentation-governance prefix omission in the new fixture rationale; the comment now uses the required `Rationale:` classification.
