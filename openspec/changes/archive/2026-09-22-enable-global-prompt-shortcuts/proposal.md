...
Implementation is expected in the bare-A1 session viewport pre-input/pointer routing and root/editor focus facade, with focused tests around `session-viewport-controller.ts`, `session-shell.ts`, and shell integration fixtures. The editor remains the authority for keybinding matching and edit history; no installed Pi patch, clipboard transport change, persisted-data migration, or `a1 pi` behavior change is intended. Physical testing rejected candidate `c50bf418` because outer runtime focus restoration did not restore the nested ordinary input surface, then rejected `a6b4d8dc` because pointer-time nested restoration still did not guarantee ownership when the later key arrived. The refined implementation restores pointer presentation and reasserts ordinary runtime keyboard ownership at each non-modal keyboard receipt before established dispatch.
