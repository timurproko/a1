...
The target timing assessment uses at least three independent first-attempt CI observations per representative class (unrelated operational, startup-sensitive, conservative-full) at identified revisions, with successful and failed samples retained. If hosted dispatch limitations prevent that complete set, final acceptance may proceed only after the maintainer explicitly dispositions the missing observations as a known gap; the missing samples remain visibly unperformed and cannot support a speedup, median, range, or completed-performance claim. Record runner label/version, Node version, cache hit/miss, selection, phase data, total runner-seconds, critical-path gate time, and workflow elapsed/available queue time. Use medians and ranges; distinguish these new observations from the two historical baseline samples, which are not a matched controlled trial. Capture comparable pre-optimization baseline measurements before the orchestration switch. Include a cold-cache control without turning repeated benchmarks into semantic retries; any failure blocks ordinary complete acceptance until diagnosed, and no attempt is discarded. A maintainer-dispositioned missing hosted cold-cache control remains a known gap rather than successful evidence and excludes the change from automatic completed-change archival.
