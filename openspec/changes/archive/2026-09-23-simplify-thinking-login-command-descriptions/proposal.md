...
- Preserve `/thinking <level>` and `/login <provider>` execution, provider argument completion, ordering, and selected-row styling.
...
- Adds focused command-row and argument-completion regression coverage and re-pins the exact startup source-byte baseline without changing command handlers, provider authentication, thinking selection, dependencies, or installed Pi code.
