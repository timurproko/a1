...
`#dispose()` will, after pointer reporting and pre-input cleanup and before `runtime.dispose()`, run the outro when the runtime is in fullscreen mode, stdout is a TTY, the effect is not `off`, and the captured frame has at least one non-blank row. Playback writes through the runtime's existing raw control path, which reaches the terminal without damage-frame arming or transformation, while a presentation freeze on the runtime drops any pinned frame the renderer still schedules until the synchronous stop sequence; each tick is one synchronized-output block, as in the prototype. The effect modules load lazily at quit so the eager startup graph does not grow. Playback is bounded by the configured duration, clamped to 300–2000 ms, plus one bounded overall guard; a rejected or thrown playback is swallowed so disposal continues with restoration.
