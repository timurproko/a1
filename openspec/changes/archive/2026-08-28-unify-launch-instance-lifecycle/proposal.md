...
Bare `a1` and prerelease `a1 pi` share the owned rendering pipeline, but each command still needs an independent per-invocation ownership model. Users must be able to launch either form concurrently, and closing one invocation must close only its complete owned process tree without manual PID or state cleanup.
...
- Use one lifecycle-only launch guardian for every interactive profile; it inherits terminal handles directly and never reads, parses, relays, or renders terminal traffic.
...
- `agent-supervision`: Replaces one exclusive foreground lease with plural boot-authenticated launch-instance supervision and plural update/reconciliation behavior.
- `launch-profiles`: Gives `a1` and `a1 pi` the same non-detachable instance lifecycle without changing their shared owned rendering pipeline or profile roots.
- `a1-shell`: Places both owned profile compositions under the same launch guardian and close contract while preserving UI-runtime terminal authority.
- `terminal-agent-runtime`: Retains generic per-instance lifecycle registration and cleanup contracts that carry no terminal bytes.

## Impact

This affects bootstrap and immutable launch entry points, profile selection, owned-UI startup, supervisor protocol and socket ownership, lifecycle domain types, SQLite schema and reconciliation, endpoint metadata, self-update fan-out, process identity inspection, platform process-tree containment, diagnostics, contract/unit/integration tests, and exact-package platform certification. The held `evolve-bare-a1-into-multi-agent-workspace` planning artifacts conflict with the approved non-detachable default and must be reconciled before that held change can resume; this change does not itself resume or implement the held multi-agent scope.
