...
- Prevent repository-local development launches from serializing a fresh Node compile cache after terminal restoration, so the parent prompt appears promptly.
...
- Affected areas: owned Pi session-shell shutdown coordination, executable completion after cleanup, repository-local launch environment, workflow-result handling, slash-command autocomplete metadata, and process/terminal lifecycle tests.
