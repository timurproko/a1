...
### 5. Keep search transitions and dropdown marks visually stable

Search results will not append a synthetic trailing spacer. While search is open, wheel reports over its entire footer—including the ruled input and bottom status row—are delegated to the result list so the pointer can remain at the bottom while reaching the final item. If bottom clamping lands exactly on an inter-section spacer, the outgoing section remains pinned and that spacer is consumed as the transition rather than spending a body row; this avoids the one-row clamp oscillation that left the real `Skills` setting below the viewport. Opening search records the current scroll position; cancelling it without editing or navigating restores that position after the temporarily shorter search viewport is removed. Editing or navigating invalidates the snapshot so deliberate search movement remains authoritative. Dropdown menus paint the effective-value checkmark separately in the accent role, then paint the remaining row with its panel or highlighted treatment so cyan does not erase active-row contrast.
