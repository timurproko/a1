...
- Add one A1 scrollbar rail with configurable appearance (`auto`, `always`, or `hidden`), style (`thin` or `thick`), and speed (`normal` at three rows, `fast` at six, or `high` at nine rows per wheel event; high combines the normal and fast rates, including for selection edge-scroll). Auto appearance responds to pointer proximity, recent scrolling, and dragging.
