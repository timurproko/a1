...
- Place the muted cycle hint directly below the heading, deduplicate level rows, keep descriptions muted and column-aligned on every row, place the active marker directly after the active level name, and omit duplicate thinking-level state from the footer while the selector is open.
...
- **[Risk] Nested row styling or column alignment can regress when selection moves or filtering rebuilds the list.** → Assert selected and unselected description cells independently, description column equality, marker order, one-occurrence rendering, and footer restoration after close.
