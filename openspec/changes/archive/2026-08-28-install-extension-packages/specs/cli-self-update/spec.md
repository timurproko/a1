...
Extension package updates SHALL live under the `pi` compatibility namespace:
`a1 pi update --extensions` for every package installed in the A1 profile and
`a1 pi update <source>` for one of them. `a1 update --models` SHALL remain top-level
for the model catalogs of the A1 profile. A package or model target SHALL NOT run
self-update, and self-update SHALL NOT update either, so each invocation does exactly one operation.
