...
- That checkpoint completed **14/28 tasks**. Remaining work included the complete attachment/conversion and oversized-data matrix, asynchronous geometry/off-screen/interleaving evidence, broader producer/parity and negative-control evidence, required implementation CI, and exact-candidate user-controlled content review. Passing that checkpoint did not establish that all reported flashing was fixed.

## Attachment ownership and payload-limit follow-up after PR #363

- PR #363 merged as `53e924c8bd754c5f79c49e4fe5a9fe9a6704f5a4`; all required checks passed for its final head `1fc007c5d7435ca07d20c10c1854c15298e5b396`, including the corrected documentation gate. The user explicitly requested a fresh implementation follow-up. Its detached worktree is `D:/Git/a1/.worktrees/preserve-agent-content-rendering-followup`, based on that current `origin/develop` commit. The older implementation worktree retains its local evidence; no other session's worktree is changed.
- The first new focused baseline was **6 failed / 10 passed**: replaced assets survived after their last use, a synchronous 256-update burst retained all 255 superseded images, and oversized ASCII/UTF-8 tool text (including separator overflow) bypassed the existing 256 KiB contract. Local synthetic evidence is under `.artifacts/rendering/attachment-baseline.log`; raw output is not published.
- `TranscriptImageAssets` now deduplicates and owns bytes independently for current transcript blocks and admitted delivery snapshots. Same-content updates reuse retained bytes without decoding/hashing per chunk. Replacement, consumption, rejected data, disposal, and session-generation changes release the appropriate ownership. The existing 16-reference and 20 MiB per-image limits remain; oversized encoded input is rejected before decoding. Mounted components retain references to their current immutable assets, not copies of the bytes, so settings rebuilds remain valid while a newer engine revision is awaiting delivery.
- Additional race regressions first failed for reentrant production during lazy-marker delivery and for authoritative removal of the source of a pending lazy block. Resolved markers now retain assets through synchronous listener delivery; full replacement seals pending snapshots before removing their source. Failure to seal within the existing 8 MiB queue budget enters the existing bounded abort/reconciliation protocol rather than deleting sources underneath the queue. A review regression also caught premature mounted-image release during a settings rebuild; the mounted resolver now preserves that independent lifetime. These review failures are recorded separately in local `attachment-races-baseline.log` and `attachment-mounted-baseline.log`.
- Tool text that cannot fit the existing UTF-8 text budget receives an explicit `Tool text unavailable` fallback instead of silently exceeding the contract or changing the operation outcome. Other supported content parts and structured details remain available. Supported accumulated output is not serialized as a raw result. Boundary descriptions are budgeted during accumulation rather than serializing an unbounded boundary array before falling back; the 3,000-part regression retains all supported text while bounding temporary metadata encoding. Tests cover large partial/final bursts, immutable complete metadata, malformed/unsupported and excess attachments, combined text/metadata limits, boundary-collapse fallback, shared assets, queued barriers, reentrancy, replacement/disposal, and queue saturation recovery.
- Local checkpoint `.artifacts/rendering/content-followup-checkpoint.json`: **362 tests passed across twelve focused suites**, including existing engine, session-shell, component, and pinned-renderer comparisons. This includes the unchanged expected wrapped-URL failure associated with #353, not a repaired link behavior. Typecheck, complete tracked-source documentation governance, architecture/product identity/provenance, strict OpenSpec validation, build, and diff checks passed. No prohibited broad local test tier ran.
- **15/28 tasks are complete**: task 3.4 is now verified. Task 3.3 remains open for the complete conversion and inline/unavailable presentation matrix; inspecting pinned Pi's image-index conversion cache identifies an additional replacement/race risk to exercise, not a verified fix. Tasks 4.3–4.5, 6.1–6.5, and exact-candidate validation/handoff/physical acceptance remain open. No terminal erase policy, renderer fork, dependency pin, comparison route, or link behavior is changed. Follow-up CI and user-controlled content review are still required; do not merge this follow-up as completed content remediation or close #353 based on this checkpoint.
