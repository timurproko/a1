...
- Approved refinement: construct raw fenced JSON containing the literal escape in `specification\u0050r`, rather than a JavaScript string that has already decoded it. Verify the raw payload contains that escape and JSON decoding yields the legacy key; version 2 must still reject it with `metadata-fields`. This specifically tests the wire representation, not a duplicate ordinary-key fixture. The maintainer approved implementation and this refinement in PR #394's handoff; approval is recorded in comment 5666569461, before any test code.
