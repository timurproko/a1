...
#### Scenario: Align the menu with the prompt
- **WHEN** bare A1 renders above-prompt candidate rows beneath a draft such as `/`
- **THEN** the selection marker column SHALL start at the same column as the `❯` prompt glyph
- **AND** each command label SHALL start at the same column as the draft text
- **AND** the gap between a command label and its description SHALL be the same as before the rows moved
