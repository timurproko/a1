...
The third manual comparison adds a user-observed ordinary-vanilla interaction requirement that intentionally differs from the pinned public `TuiAltScreen` defaults used by the automated fullscreen producer. Source tracing shows that pinned `TuiAltScreen` currently owns mouse selection, copies automatically on button release, calls `flash("Copied!")`, reapplies inverse video after embedded ANSI sequences so source colors remain visible, and defaults `wheelScrollLines` to one. In ordinary `a1 pi`, the user instead observes a retained uniform inverse selection, no copy flash, `Ctrl+C` copying only while a selection is active, and three rows of movement per wheel notch.

AddOne will preserve fullscreen ownership and nested/primary scroll routing while matching that ordinary vanilla interaction: mouse release retains but does not copy the selection; selected content is rendered with a uniform inverse style after suppressing underlying foreground/background colors; `Ctrl+C` copies selected plain text and consumes the interrupt only when a selection exists; no copy notification is rendered; and each wheel notch requests three rows before normal nested-scroll and overscroll chaining. This is an approved, user-directed deviation from the pinned fullscreen helper defaults, not a comparator tolerance. It must be recorded in the source ledger with the affected acceptance cases. Installed Pi code SHALL NOT be patched or deep-imported; if no public option can express the behavior, the minimum coherent selection-owning source unit and dependencies SHALL be provenance-recorded and mechanically ported behind the existing TUI adapter boundary.

This finding reopens the TUI runtime, source-coverage, independent parity, and terminal-lifecycle tasks it contradicts. Acceptance requires focused press/drag/release, styled-content selection, selected and unselected `Ctrl+C`, absence-of-flash, one-notch three-row, nested-scroll, boundary, resize, and restoration tests plus a fresh user comparison.
...
The same comparison also shows that selection styling remains under-tested. The current terminal-write regular expression only recognizes selected inverse spans within one write and the existing test checks one multi-character colored sample after concatenating writes. It does not prove one-character, double-click word, line, arbitrary-area, embedded-reset/inverse, or differential-write behavior, and it does not guarantee the user-requested bright-white background. Selection highlighting SHALL move to a coherent stateful/source-ported boundary that parses complete SGR transitions, applies dark text on one uniform bright-white inverted background, and restores following styles correctly. Acceptance SHALL inspect raw per-write ANSI and rendered terminal cells for every granularity across plain, colored, bold, linked, inverse, Unicode, whitespace, and multi-row content while retaining no-copy-on-release, selected `Ctrl+C`, no-flash, and resize/scroll behavior.
...
The sixth manual comparison exposes two additional approximation boundaries. First, retaining Pi's screen-coordinate selection while rewriting inverse ANSI only at the terminal `write()` seam allows source foreground colors to survive differential rendering and allows the old rectangle to move onto newly rendered autocomplete and command rows. Ordinary vanilla interaction retains the selection after release, but a non-copy keyboard action resolves it before the key changes editor or UI state. AddOne SHALL therefore clear an active selection before forwarding any non-copy keyboard input, while selected `Ctrl+C` retains priority and unselected `Ctrl+C` remains interrupt. Opening autocomplete, commands, selectors, dialogs, or any replacement input surface SHALL NOT inherit an old selection. Uniform styling SHALL be implemented at a coherent stateful SGR/cell boundary and tested from final emulated terminal cells after incremental writes; concatenated full-redraw ANSI alone is insufficient evidence. The seventh manual comparison further proves that per-line inverse rewriting is not equivalent to ordinary vanilla selection: the selected stream must form one continuous block through trailing cells and blank rows, remain uniformly dark-on-bright-white while wheel scrolling, and be fully removed before command/autocomplete replacement content renders.
