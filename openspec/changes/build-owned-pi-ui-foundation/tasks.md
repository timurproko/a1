...
## 7. Build the Complete Public Pi Shell and Prove Current-Version Parity
...
- [x] 7.2 Inventory the complete pinned Pi interactive baseline—startup composition, stateful components, editor/autocomplete/keybindings, built-in commands, prompt loop, selectors, settings, sessions, models, thinking, events, tools, footer/status, clipboard, resize, errors, and shutdown—and bind every behavior to exact upstream source provenance plus an independently verifiable acceptance case; explicitly record the failed demonstration-shell smoke result and invalidate its parity evidence.
- [x] 7.3 Rebuild `PiSessionShell` startup composition and persistent visual tree from public Pi components plus provenance-recorded controller ports, including vanilla header/notices, transcript spacing, stateful editor, command autocomplete, keybinding hints, status indicators, and footer; pass separate pinned-upstream versus AddOne captures at fixed widths and startup states.
- [x] 7.4 Port the complete pinned built-in command and input workflow manifest, including prompt templates, skills, command autocomplete, bash input, model/thinking controls, settings, session/new/resume/fork/tree flows, login/logout, import/export/share/copy, compaction/reload/debug/help-style surfaces, clipboard, queue behavior, and shutdown; pass exhaustive manifest, routing, selector, success, cancellation, and failure conformance against independently recorded upstream outcomes.
- [ ] 7.5 Port the pinned session-event orchestration with persistent streaming assistant and tool components, user/custom messages, retries, compaction, errors, queue updates, terminal progress, transcript rebuild, and real prompt completion; pass synthetic edge cases plus a real SDK-backed prompt integration proving visible submission, streaming, completion, and error reporting.
- [ ] 7.6 Replace self-generated static snapshots with an independent two-producer component/composition parity harness for pinned Pi and AddOne at fixed widths and states, preserving raw outputs and failing on visible row, spacing, wrapping, editor, selector, status, footer, error, or resize divergence outside documented terminal-only tolerances.
- [ ] 7.7 Replace synthetic-only event/frame evidence with independent pinned-Pi and AddOne command-manifest, scripted-event, real-prompt, and captured-ANSI producers; compare state transitions and screens, preserve both raw artifacts and hashes, and prove the harness fails when a command, prompt effect, row, or transition is intentionally removed.
- [ ] 7.8 Correct every parity divergence, rerun focused and containing gates, and record the exact source commit, package versions, full behavior coverage, exclusions, tolerances, raw artifact hashes, and machine-readable independent parity evidence before requesting fresh manual acceptance.

## 8. Accept the Parity-Safe Base UI

- [ ] 8.1 Run complete automated gates after the complete Pi shell migration: build, typecheck, tests, architecture, SDK/runtime/component conformance, independent component/command/event/real-prompt/frame parity, lifecycle, resource, packaging, audit, and strict OpenSpec validation; preserve machine-readable evidence and reject the invalidated demonstration-shell parity record.
