...
- Strengthen ordinary-vanilla selection parity so character, word, line, and arbitrary-area selections always render dark text on a uniform bright-white inverted background without syntax-color leakage, regardless of selected length, source ANSI, or differential-write boundaries. Retain a selection after mouse release only until copy or another ordinary keyboard action resolves it; clear it before forwarding non-copy keyboard input so stale screen coordinates never select newly opened autocomplete, command, selector, dialog, or editor content.
