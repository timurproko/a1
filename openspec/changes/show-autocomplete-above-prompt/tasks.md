...
- [ ] 1.2 Introduce shared typed editor-body/menu geometry in the owned presentation boundary, preserving the existing editor/history paths and source attribution; verify body boundaries, prefix, border indicators, and cursor markers for single-line, wrapped, scrolled, and Unicode drafts without private dependency access.

## 2. Compose autocomplete above the prompt

- [ ] 2.1 Render default-editor completion rows above the upper border only in bare A1, preserving list order, styling, pagination, and key semantics; verify command, argument, path/resource, and extension-provider completions use the same placement with history enabled and disabled.
- [ ] 2.2 Allocate menu capacity after non-menu dock layout, with selection-aware clipping, auxiliary-row priority, zero-capacity state retention, and live visible-item limits; verify full, short, one-menu-row, zero-menu-row, resized, and narrow-terminal frames remain bounded without menu-caused input displacement.
...
- [ ] 3.4 Record the named bare-A1 autocomplete presentation exception in existing compatibility evidence and verify independent `a1 pi`/untouched-Pi comparisons retain pinned rendering and semantics without package mutation or widened parity normalization.
