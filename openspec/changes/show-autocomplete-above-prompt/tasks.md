...
- [ ] 2.4 Add exactly one horizontal top line directly before visible autocomplete, matching the prompt border's glyph, full width, and current color; include it in the shared body offset and remove it with the menu. Verify both history modes, theme/editor-mode color changes, resize, no stale line after dismissal, and non-text pointer behavior without reducing completion-item limits or changing pagination.
