...
- [x] 2.1 Move the existing rendered completion block above the upper border only in bare A1, preserving list order, foreground styling, pagination, and key semantics; verify command, argument, path/resource, and extension-provider completions use the same placement on both existing editor paths.
- [x] 2.2 Preserve existing menu sizing, selection window, visible-item setting behavior, and terminal clipping rather than introduce a capacity budget; verify configured limits and resize/narrow-terminal cases retain their existing behavior with only row placement and menu background/cell-padding decoration changed.
