...
- Give the visible menu a top line matching the prompt border's current color and width, without changing the completion rows or their original background.
...
`existing pre-input dock content → above-editor widgets → menu top line → autocomplete → upper border → prompt text → lower border → below-editor widgets → footer`

Render one horizontal line only when the menu has rendered rows, directly before those rows with no blank spacer. Use the prompt border's horizontal glyph, full rendered width, and current border-color function, including theme and editor-mode color changes; do not hardcode a palette value or copy history/scroll labels from the input border. Keep the input's existing upper border between the menu and prompt.

This restores the top-line design in place of the rejected background panel. Keep candidate rows intact, including their original background and padding; remove menu-specific background painting rather than combining it with the line. Relocate the existing counter from its trailing row into this top line: `─── 1/24 ─…`, using the history label's four-cell inset and the same dim style, without the word `History` or parentheses. Preserve the counter's existing selected-item/total meaning and visibility conditions. Remove the trailing counter row, with no blank replacement. A fitting list that does not emit a counter keeps a plain top line.

The shell continues to bottom-align this dock. For unchanged non-menu content, adding `m` candidate rows plus the top line decreases the dock's start row by `m + 1` while increasing the editor-body offset by exactly `m + 1`; the editor's terminal row is therefore unchanged. Closing removes both the menu and its line, and filtering gives unused rows back to the transcript viewport.
...
Capture counter presentation through the existing select-list `scrollInfo` theme callback, whose pinned contract emits the final counter row. This supplies explicit row-ownership metadata without scanning the rendered menu for labels or border glyphs. Reset that metadata before each render so filtering, cancellation, or replacement cannot retain a stale counter. Strip only the known counter wrapper; do not recompute selection or pagination. On widths too narrow to obtain or fit a complete counter, omit the label and retain the bounded line rather than fabricate a truncated count.
...
Use both existing editor paths as regression coverage, not as a history redesign. Cover command/argument/path/extension completions; Unicode and narrow widths; resize under the existing clipping rules; existing visible-item setting behavior; below/above-editor widgets; prompt selection/copy; extension replacement/restore; and detached or streaming transcripts. Verify the top line's width and current border color after theme/mode changes and resize, and include it in geometry and non-text pointer assertions. Inspect final terminal cells after shrinking and closing to catch stale menu or line rows. Verify candidate rows retain their original ANSI styling and padding, with no panel shading. Compare counter values with the independently rendered pinned list across navigation, filtering, visible limits, and asynchronous updates; assert the original counter row is absent and the top label matches history inset/color in both history modes. Fixed-height list navigation must preserve existing dock-only transcript reuse. Maintain an independent `a1 pi`/pinned comparison proving no upstream presentation change.
