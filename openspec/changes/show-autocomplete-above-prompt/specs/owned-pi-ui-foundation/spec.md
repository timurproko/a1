...
Bare A1 SHALL declare above-prompt autocomplete as a placement-only replacement for the ordinary editor's pinned below-prompt list. This named exception SHALL supersede pinned row-order and resulting editor-anchor parity only for that surface. Menu sizing and clipping, editor choice, history, contextual suggestions, settings, extensions, and unrelated shell behavior SHALL retain their existing contracts.
...
- **AND** only bare A1's explicitly declared placement differences SHALL be treated as expected autocomplete deviations
