...
- `owned-pi-ui-foundation`: Declare bare A1's history replacement and its precise exception to pinned history behavior while preserving the `a1 pi` comparison.
...
- Existing session-shell submission capture, editor history synchronization, semantic editor-border presentation, and A1 settings declarations/application wiring will be affected during implementation. No extension monkey-patching or private Pi distribution imports are required or permitted.
