...
- Keep timeouts finite and workload-specific while making ordinary tests retain their strict default.
- Make partition selection and timing visible in plans, outcomes, and regression evidence.
...
- Increasing the global Vitest timeout or serializing the entire fast suite.
...
### 2. Execute the partition once with bounded serialization

The planner will emit a distinct `vitest-fast-resource-sensitive` invocation using the exact declared paths, no file parallelism, and a 30-second per-test timeout. Thirty seconds matches the existing bounded non-isolated full-suite allowance while remaining far below integration and rendering budgets. Tests within each file remain non-concurrent under the repository Vitest configuration.

The ordinary `vitest-fast` invocation retains file parallelism and its existing default timeout. The dedicated invocation runs on every platform, not only Windows, so plan semantics do not depend on the host and cross-platform evidence stays comparable.

Alternative: set the complete fast tier to `--testTimeout=30000`. Rejected because it masks hangs in hundreds of ordinary tests without reducing contention. Alternative: run the complete fast tier without file parallelism. Rejected because it discards safe concurrency and unnecessarily lengthens every validation lane.
...
Focused planner tests will assert command identifiers, exact exclusions, no-file-parallelism, the finite timeout, test disjointness, and the absence of retry behavior. A focused execution regression will exercise the resource-sensitive partition and record structured evidence from repeated clean runs. Repository governance will ensure future partition edits cannot omit, duplicate, or silently return a test to contention.
...
- **[Risk] Thirty seconds could delay detection of a genuine hang.** → Apply it only to the reviewed partition, retain process output, and fail at the finite bound without retry.
- **[Risk] Tests in one partition can still contend internally.** → Repository Vitest sequencing remains non-concurrent, and any repeated timeout after isolation is treated as a real workload/budget issue rather than retried.

## Migration Plan

1. Extend validation-suite parsing and planning with the declared resource-sensitive fast partition and invariants.
2. Add the initial evidence-backed file list and focused planner/governance coverage.
3. Run focused tests, typecheck, architecture, documentation governance, and strict OpenSpec validation; rely on CI for ordinary and exact-package suites.
