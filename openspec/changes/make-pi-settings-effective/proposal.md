...
- Preserve the owned settings interaction contract on the shared component layer: `/` alone invokes a ruled search input, ordinary typing outside that input does not search, the status bar is derived from active shortcut declarations, selected-entry descriptions remain model metadata rather than rendered rows, and wheel movement uses the effective live `scrollbarSpeed` policy instead of a settings-screen literal.
...
- `owned-ui-settings`: agent settings gain declared application timing and availability, the screen reports applied or deferred outcomes while omitting unavailable options, and its rows, values, menus, notices, and controls retain reviewed shared styling while search invocation, ruled input, status hints, description visibility, and wheel cadence follow the owned settings UX.
