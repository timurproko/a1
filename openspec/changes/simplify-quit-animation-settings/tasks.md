...
- [ ] 3.2 Hand off the exact built candidate for a physical check that the settings screen shows `Quit animation` under `Generic` and no `Quit` section, and that `/quit` plays the fall effect with the switch on and returns immediately with it off; record the outcome.
