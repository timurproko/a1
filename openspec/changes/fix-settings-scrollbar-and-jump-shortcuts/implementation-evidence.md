...
| `npx vitest run test/repository-governance/owned-settings-interaction-boundary.test.ts test/repository-governance/shortcut-hint-governance.test.ts test/repository-governance/pi-session-shell-provenance.test.ts test/ui test/features/owned-ui` | 41 files, 476 passed. |
...
- The settings rail asks `scrollbarPresentation` with no activity window, so `auto` reveals on rail hover or drag only, exactly as the transcript rail does today; no timer was added.
