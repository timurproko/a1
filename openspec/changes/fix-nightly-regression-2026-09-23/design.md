...
- The first repair hypothesis was hosted-runner capacity exhaustion. PR Full regression run `35897923519` disproved it: macOS received the same runner shutdown one second after Vitest started with the ordinary partition explicitly capped at two workers. Retain the cap only while narrowing the active file set; it is not accepted as the root-cause fix.
...
- Known gap: the terminating macOS test module is not yet identified because the default reporter records only completion. The next exact-head PR Full regression includes start-boundary evidence and remains a diagnostic run, not handoff proof.
