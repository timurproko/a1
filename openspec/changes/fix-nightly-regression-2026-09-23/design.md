...
- After merging current `develop`, exact-head run `35917411661` exposed two contract updates rather than product regressions: the validation-suite manifest test still expected 21 resource-sensitive files, and the newly merged prompt shortcut test expected `alt+up` instead of the rendered macOS `option+up`. Both focused suites pass after making those expectations match their authoritative platform/configuration inputs.
