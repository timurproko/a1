...
The Release run of 2026-09-23-155 failed on `develop` at `c37f420` (https://github.com/timurproko/a1/actions/runs/35838366882).  Orchestration failures: lane darwin-node24 in job `Validate darwin-node24`; lane linux-node24 in job `Validate linux-node24`; lane Publication result in job `Publication result`. The nightly triage opened this change so the fix starts from the recorded evidence instead of the failure email.
...
Recorded in the pull-request body: failed commands per lane, their test files, a bounded log excerpt, and the `develop` commits since the last successful run (31 since `7d26554`).
