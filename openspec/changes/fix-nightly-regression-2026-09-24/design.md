...
- Keep the event head as immutable authority, but read mutable body and draft metadata from GitHub at readiness execution. Current metadata is accepted only when its head still equals the event head; a superseded head is deferred, and unavailable or malformed current metadata fails closed. The trusted base copy remains dependency-free and requests only pull-request read permission.
...
- `npx vitest run test/repository-governance/development-validation-readiness.test.ts test/repository-governance/impact-aware-validation-workflows.test.ts test/repository-governance/pr-full-regression.test.ts` passed all 64 readiness, workflow, and selection tests.
