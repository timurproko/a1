...
On 2026-09-11, the maintainer approved retaining A1's accurate missing-`gh` diagnostic after reviewing the vanilla-Pi/A1 `/share` comparison. This amendment declares only that wording exception; installed-but-unauthenticated, export, gist-creation, parse, cancellation, and success outcomes remain subject to their existing pinned-parity requirements. It does not approve final implementation acceptance or archival.

The CLI and interactive-outcome milestones already merged in PRs #250 and #251. Task 4.8 is reopened to verify the discovered exception against actual pinned behavior. Unmerged presenter work is not included in this specification-only amendment, and no other task is marked complete by it.
...
- Retain `Error: GitHub CLI (gh) is not installed. Install it from https://cli.github.com/` when `/share` cannot find the `gh` executable. This is a named exception to Pi 0.84.2's misleading `Error: GitHub CLI is not logged in. Run 'gh auth login' first.` for that condition. Keep the latter message for an installed but unauthenticated GitHub CLI, and preserve error presentation and stopping before export/upload in both cases. No general permission to improve other pinned messages is introduced.
...
- `owned-pi-ui-foundation`: Specify command-outcome message coverage, concrete divergent states, style/layout-sensitive evidence, and the single missing-GitHub-CLI wording exception within existing declared customization boundaries.
