...
- [ ] 4.1 Represent route-specific message severity and multi-message outcomes explicitly through existing owned workflow boundaries; verify warning/status/error selection and partial-success ordering do not depend on parsing message prefixes.
...
- [ ] 4.6 Preserve import error context through confirmation and missing-cwd recovery; verify usage, success, error, declined confirmation, and extension cancellation messages or silence against pinned behavior.
...
- [ ] 6.2 Complete the independent interactive message-cell matrix across all inventoried outcomes, relevant themes, ordinary/narrow widths, both padding values, and state transitions; verify negative/mutation cases reject wrong punctuation, severity, style, missing rows, wrapping, and overbroad normalization.
- [ ] 6.3 Update applicable source provenance and integrate focused coverage with the repository's existing required validation selection; verify architecture/public-boundary checks and required CI evidence without introducing a second runtime or a disconnected parity inventory.
- [ ] 6.4 Verify integration invariants across CLI and interactive routes: isolated A1 package/model profile, no CLI supervisor/UI launch, preserved child output, unchanged A1 self-update/version behavior, and declared UI replacements/customizations; record the required CI result and any remaining gaps.
- [ ] 6.5 Provide the exact implementation artifact and focused commands for user-controlled Windows Terminal/Git Bash review of both model-refresh aliases and representative interactive success/empty/error/multiline states; verify the screenshot now has exactly Pi's green summary and record explicit user acceptance before claiming completion.
