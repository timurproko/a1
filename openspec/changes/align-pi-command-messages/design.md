...
| Missing `gh` during `/share` (2026-09-11 amendment) | Engine workflow adapter and owned error presenter | Published Pi 0.84.2 misclassifies native missing-executable failure as not authenticated. Retain A1's accurate installation guidance as one named wording exception; preserve exact parity for other share outcomes. |
...
The 2026-09-11 follow-up verified that Node's native missing-executable result has `status: null` and `error.code: "ENOENT"`. Pi 0.84.2's published `handleShareCommand` checks `spawnSync("gh", ["auth", "status"]).status !== 0` before its exception handler; that native result therefore produces the not-logged-in message, not installation guidance. An isolated invocation of the unchanged method with this result reproduced the mismatch without a real GitHub command, export, or gist upload. A1's existing asynchronous process boundary instead rejects missing `gh` with ENOENT and reports installation guidance. The maintainer approved keeping that accurate diagnosis after reviewing the comparison table. This is planning evidence for the narrow exception, not a claim that the remaining outcome matrix or final acceptance is complete.
...
For missing `gh` only, keep the existing A1 user-visible error `Error: GitHub CLI (gh) is not installed. Install it from https://cli.github.com/`. Do not copy Pi 0.84.2's `Error: GitHub CLI is not logged in. Run 'gh auth login' first.` into that branch. The latter remains the required message when `gh` exists but authentication fails. Keep the backend's unprefixed semantic error and the presenter's single `Error:` prefix, error color, padding, wrapping, and session lifetime; both preflight failures stop before exporting or creating a gist. Do not auto-install `gh`, initiate login, add a dependency, change vanilla Pi, or classify permission and other non-missing-process failures as missing executables. Export/gist/parse failures, cancellation, and successful links retain their existing parity contract.
...
The missing-`gh` fixture must exercise Pi's native non-throwing ENOENT result shape (`status: null` with an error) as well as A1's corresponding asynchronous missing-process rejection; making Pi's process stub throw would exercise a different branch and conceal the defect. Preserve the independently captured pinned not-logged-in text and require A1's exact installation text. Add an installed-but-unauthenticated nonzero result and non-missing-process negative cases so the exception cannot hide unrelated failures. No real `gh`, credential access, export, or gist operation is needed to establish this preflight evidence. Keep this case in the existing cumulative outcome inventory, tied to its exact source/pin and error-presentation checks; re-review it on an upstream behavior change rather than silently widening normalization.
...
No configuration or persisted-data migration is required. The visible output correction intentionally removes A1-specific model-refresh summaries and makes supported explicit package help succeed; existing operation and syntax exit-code contracts otherwise remain intact. The missing-`gh` amendment preserves an existing A1 diagnostic and requires honest exception evidence rather than a new runtime feature. Integrate the amendment as an OpenSpec-only PR before separately authorized implementation/evidence work resumes in a fresh stream; do not include the unmerged presenter changes in the amendment. Specification approval precedes focused implementation changes: first establish the CLI/package slice on current integration, then deliver interactive outcome and presenter slices while maintaining one cumulative inventory and acceptance contract. The OpenSpec change remains active until all required slices are integrated and accepted. After code validation, compare each exact runnable artifact in the terminal before accepting that slice. If a correction regresses command behavior or lifecycle, revert the affected implementation commit and reopen its inventory outcomes; never restore a false parity claim by normalizing the difference away.
