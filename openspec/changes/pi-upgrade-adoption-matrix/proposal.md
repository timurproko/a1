...
- Make the sync safe to re-run: the workflow does not push when the remote proposal branch carries commits the bot did not author and comments the fresh report on the pull request instead; the body refresh rewrites only the report between `<!-- pi-upgrade-report -->` markers; an existing OpenSpec scaffold is kept; dispatch gains `refresh` to re-run the steps on top of an existing proposal branch. Closing a proposal pull request with the `pi-upgrade-skipped` label skips that version: the workflow reads closed proposals with the label and the driver proposes the newest published version newer than the pin that is not skipped; a closed proposal without the label is proposed again, manual dispatch with `version` ignores skips, and an optional `PI_UPGRADE_FREEZE_UNTIL` repository variable silences the schedule until a date.
