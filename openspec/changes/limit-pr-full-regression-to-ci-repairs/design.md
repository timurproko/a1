...
The inline pre-deployment bootstrap no longer conservatively runs Full regression for every non-planning candidate. If the selector is unavailable on a future base, bootstrap records no complete-suite selection and leaves ordinary Development's existing fail-closed routing/aggregation responsible for validation. Bootstrap cannot claim generated-repair provenance it cannot verify.
...
- Current-head freshness fixtures for author, body, head, base, readiness, provenance, and history changes.
