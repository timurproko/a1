...
- Escape and the conventional Ctrl+C interruption alias abort startup silently with exit code 130 before project settings, resources, or the owned shell are constructed.
...
From an uncovered folder under `defaultProjectTrust: ask`, start the development checkout. Press Escape and confirm A1 returns directly to the parent terminal without starting the shell, printing a crash, or flashing an intermediate restricted session. Relaunch, choose Do not trust, and confirm A1 starts normally without a warning.
