...
- **THEN** the diagnostic SHALL use display name `A1` and SHALL NOT display `AddOne` or `addone` as the current product name
...
- **WHEN** the environment supplies an `ADDONE_*` variable without the corresponding `A1_*` variable
...
- **WHEN** A1 encounters a release manifest, endpoint record, protocol frame, or evidence schema using the legacy `addone` namespace
...
- **THEN** public and internal executable entry filenames included by A1 SHALL use `a1` and SHALL contain no current `addone`-named artifact

### Requirement: Historical identity references remain factual and isolated
Archived changes, immutable historical evidence, and explicit obsolete-package rejection or deprecation fixtures MAY contain `AddOne`, `addone`, `ADDONE_*`, or `@timurproko/addone`. Such references SHALL NOT be imported, rendered, generated, or accepted as current A1 identity.
