...
- [ ] 1.1 Add an `AgentPackagesPort` to `agent-engine-contracts` covering install,
...
- [ ] 1.2 Cover the contract's shapes in `test/foundation/agent-engine-contracts`

## 2. Implement the port on the Pi boundary

- [ ] 2.1 Implement the port in `pi-engine-adapter` over pinned Pi's public
...
- [ ] 2.2 Do not delegate to Pi's own package command handler: it prints Pi's command
  names and exits the process, both of which A1 owns
- [ ] 2.3 Translate Pi's progress and failures into port results, so nothing that
  reaches the terminal is written in Pi's voice
- [ ] 2.4 Cover install, remove, update, and list against a temporary profile root in
...
- [ ] 3.1 Parse `install`, `remove`, `uninstall`, and `list` in `src/cli/dispatch.ts`,
  and narrow the existing "commands take no further argument" rule to the launch forms
- [ ] 3.2 Parse `update` with `--extensions`, `--models`, a positional source, and the
  `self` alias; refuse `pi` with the pinned-release explanation
- [ ] 3.3 Resolve the A1 profile root through the launch feature's existing path
  resolution and initialize it before use
- [ ] 3.4 Wire a package handler in `bin/cli.js` beside launch, version, and update,
  so the CLI owner keeps no Pi import
- [ ] 3.5 Say on success that a running session needs a restart to load the change
- [ ] 3.6 Extend the usage text with the new forms
- [ ] 3.7 Cover the parse table in `test/cli`, including the `uninstall` alias, the
  refused `update pi`, and a rejected profile argument

## 4. Document it

- [ ] 4.1 Add the package commands to the README command list, in the same shape as
