...
Candidate activation is a release-cohort replacement transaction:

1. Verify the candidate release and run its packaged certification gates.
2. Record it as pending while the old cohort remains authoritative.
3. Ask the old release's own coordinator code to drain and report generation recoverability.
4. Defer automatically while any non-resumable PTY remains live; keep launching the matching old UI.
5. Capture durable state and stop or migrate recoverable workers under their existing lease rules.
6. Confirm endpoint and process ownership release.
7. Atomically switch the active-release pointer and start the new supervisor/UI cohort.
8. Retain the prior release for rollback until no durable reference or retention policy requires it.

Stale recovery validates endpoint handshake, boot nonce, PID start identity, immutable release identity, and durable generation ownership. A dead owner is cleaned automatically. An unresponsive owner with provably no live generation receives bounded graceful shutdown followed by platform-native process-tree termination; this uses direct process APIs and never asks users to adapt `taskkill` syntax to their shell. If ownership of a live non-resumable generation is uncertain, AddOne preserves it and reports blocked recovery instead of risking silent work loss.

Alternative considered: copy Pi's direct global package overwrite. Rejected because Pi has no detached protocol peer while AddOne does. Alternative considered: negotiate every new UI with every historical supervisor. Rejected because it creates an unbounded compatibility matrix; selecting the supervisor's matching retained UI makes cross-release compatibility unnecessary during normal operation. Alternative considered: always kill the old supervisor during update. Rejected because it destroys non-resumable PTYs and recreates the manual intervention problem.
...
Installed clients select channels explicitly. `addone update` and `a1 update` continue to resolve `latest`; `addone update next` and `a1 update next` resolve `next` and install the exact version returned by that tag. Using the same `next` name in AddOne and npm avoids another channel alias. This prevents stable users from entering the preview channel accidentally while making repeated preview updates one command.
...
A separate N−1 AddOne transition matrix starts a retained old supervisor with both idle and busy worker cases, installs the packaged candidate, invokes the bootstrap, and asserts release-matched UI selection or safe activation, no generic malformed-message error, no duplicate owner, durable pending activation for non-resumable PTYs, automatic completion after blockers exit, and bounded stale-owner cleanup. Candidate process inventories must prove execution from immutable package content rather than the development checkout.
...
- **[Risk] Candidate activation can remain pending indefinitely behind a non-resumable PTY.** → Keep the old cohort fully usable, expose the blocker and explicit stop action, and activate automatically as soon as the blocker exits.
...
- Implement safe idle activation, busy non-resumable deferral, automatic activation after blocker exit, and bounded stale-owner reconciliation without shell-specific kill instructions.
