...
Stable background candidate activation and explicit preview replacement are separate policies over the same durable release transaction. Stable launch remains preservation-first and may defer behind a live non-resumable PTY. By contrast, invoking `update:next` is explicit consent to stop every verified AddOne-owned session and activate immediately.

The preview stop-install-activate transaction is:
...
Installed clients select policies explicitly. `addone update` and `a1 update` continue to resolve stable `latest` under preservation-first activation. `addone update:next` and `a1 update:next` resolve `next` and execute the immediate stop-install-activate transaction. The colon form mirrors `publish:next`, distinguishes destructive preview replacement from stable update, and is the complete user operation: there is no required `status`, `shutdown`, `--restart`, PID lookup, process-kill command, or state-directory deletion.
...
- **[Risk] Stable candidate activation can remain pending indefinitely behind a non-resumable PTY.** → Keep preservation-first stable activation deferred, while the separately explicit destructive `update:next` command stops verified blockers and activates immediately.
...
- Implement safe idle activation and busy non-resumable deferral for stable launch, plus immediate ownership-verified stop-install-activate behavior for `update:next`, boot-scoped generation reconciliation, transaction recovery, and bounded stale-owner cleanup without shell-specific kill instructions.
