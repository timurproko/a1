...
A passing rerun, manual confirmation, or implementation-only change is not completion evidence. The publication-gate normal-screen handoff regression additionally proved that the first ready snapshot may legally contain different occupied-row counts and later report correspondingly different full-viewport scroll distances; host-origin accounting must therefore derive physical placement from the actual rows appended and scrolls subsequently consumed rather than assuming one startup transaction boundary. If exact platform behavior cannot be made deterministic at unit level, a deterministic model/oracle test plus the isolated platform scenario is required. Every added or modified test must itself be executed and pass together with the containing build, check, release, packaging, or publication command before its task or coherent subtask is marked complete or committed; a focused pass does not excuse a failure elsewhere in that required command. The user does not need to request regression coverage or test execution separately.
