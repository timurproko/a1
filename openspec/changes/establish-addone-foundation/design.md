...
A passing rerun, manual confirmation, or implementation-only change is not completion evidence. Bug-fix validation proceeds in fixed order: preserve evidence; create and observe a failing deterministic simulation; correct the cause; pass that simulation plus neighboring boundary simulations under production defaults; pack one exact tarball; install and launch that tarball in an isolated production-default certification; run the complete release gate; and only then upload the same certified bytes. Test-only timeout, environment, argument, dimension, or configuration overrides cannot stand in for production-default certification, and retry-until-green never establishes validity. Machine-readable evidence binds each stage to source commit, package version, tarball integrity, platform, runtime identity, and production/default override inventory. The publication-gate normal-screen handoff regression additionally proved that the first ready snapshot may legally contain different occupied-row counts and later report correspondingly different full-viewport scroll distances; host-origin accounting must therefore derive physical placement from the actual rows appended and scrolls subsequently consumed rather than assuming one startup transaction boundary. If exact platform behavior cannot be made deterministic at unit level, a deterministic model/oracle test plus the isolated platform scenario is required. Every added or modified test must itself be executed and pass together with the containing build, check, release, packaging, or publication command before its task or coherent subtask is marked complete or committed; a focused pass does not excuse a failure elsewhere in that required command. The user does not need to request regression coverage or test execution separately.
