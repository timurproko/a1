...
- [ ] 1.118 Preserve the installed `0.1.5-dev.5` cursor-only screenshot and runtime/database evidence, deterministically reproduce that release tests hid ordinary-launch failure by overriding the 8-second production readiness default to 15 seconds, adopt one production default that passes constrained cold-start simulation, and require affected startup simulations to exercise that default.
