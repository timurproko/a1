...
- [ ] 1.117 Preserve the publication-gate evidence where different valid initial Pi snapshots caused the first later full-viewport scroll to place editor text one row below direct Pi, add deterministic handoff tests covering the observed caller row plus 14- and 16-row startup scroll variants, correct generic normal-screen origin accounting without Pi/content inspection, pass the focused test, packaged real-Pi scenario, and complete check gate, then publish the exact `0.1.5-dev.5` candidate before resuming preview validation.
