...
- [x] 1.106 Ensure every persistent AddOne process and native module executes only from its verified immutable release root before npm replacement, including migration detection and actionable automatic handling for legacy mutable-install supervisors.
- [x] 1.107 Add transaction-journal recovery and rollback for interruption after shutdown intent, ownership release, npm installation, materialization, certification, active-reference commit, and new-supervisor startup, proving every rerun converges to exactly one verified active or rollback cohort without mixed ownership.
...
- [x] 1.109 Add equivalent dependency-light `addone version` and `a1 version` commands that always show the invoked package's valid `Installed` version, query and show exact npm `latest` as `Release` and npm `next` as `Next`, preserve installed output with unavailable remote fields and concise diagnostics when registry lookup fails, and prove no UI, supervisor, PTY, native addon, database, release state, or update transaction is loaded or mutated.
- [ ] 1.110 Publish the completed unified immediate-update and version-stat correction as the next immutable Windows-tested `-dev.N` candidate, install from npm `next`, manually confirm the preview workflow is only `npm run publish:next`, `a1 update:next`, and `a1`, verify `a1 version`, and verify the same no-manual-cleanup behavior for stable `a1 update` against an isolated `latest` transition.
- [x] 1.111 Make regression-first correction a permanent project rule and encode deterministic cause-specific coverage for the recently observed stale cursor/mode oracle, full-frame polling latency inflation, packaged-gate resource contention, synchronized marker-only publication, answer-before-settled-status checkpoint race, and acknowledged-but-nonexiting supervisor activation failure; require affected mandatory gates before completion, and preserve exact online, offline, and both-alias version-output tests.
- [x] 1.112 Restore `version` display order to `Installed`, `Release`, then `Next`, enforce that exact order for online, offline, `addone`, and `a1` output, recognize the registry-confirmed `0.1.5-dev.3` upload despite npm's post-upload browser-auth E403, and require a new immutable `0.1.5-dev.4` preview candidate for the restored order.
- [x] 1.113 Make registry identity authoritative when npm's browser-auth completion poll fails after a successful immutable upload: deterministically cover the observed E403-after-upload case, verify exact version/tag/artifact before success, preserve combined publish/verification diagnostics when upload is absent, and prevent instructions to republish an already registered version.
- [x] 1.114 Preserve the observed Windows real-Pi duplicate-footer screenshot, prove supervisor resident state contains one footer pair while a later normal-screen resize/resynchronization snapshot leaves stale physical rows, make every replacement snapshot atomically overwrite newly blank rows without Pi/content inspection, and add deterministic moved-footer plus packaged real-Pi resize/reflow regressions requiring exactly one status/footer pair.
- [x] 1.115 Require every added or modified test to be executed successfully both focused and through its containing build/gate before completion or commit, preserve the packaged multi-CLI outer-PTY exit timeout as regression evidence, prevent optional post-restore Windows console-mode evidence from synchronously blocking foreground CLI exit, and pass the exact complete release-gate command that exposed the failure.
- [x] 1.116 Preserve the observed vanilla-Pi-below-prompt screenshot and the hosted flash-clear-repaint evidence, make initial generic normal-screen handoff append its occupied resident rows from the current physical cursor with natural bottom-edge scrolling and no home/clear while retaining alternate-screen atomic clear and later replacement blank-row cleanup, and pass deterministic startup plus packaged real-Pi no-clear regressions through their complete required gates.
- [ ] 1.117 Preserve the publication-gate evidence where different valid initial Pi snapshots caused the first later full-viewport scroll to place editor text one row below direct Pi, add deterministic handoff tests covering the observed caller row plus 14- and 16-row startup scroll variants, correct generic normal-screen origin accounting without Pi/content inspection, and pass the focused test, packaged real-Pi scenario, and complete check gate.
- [x] 1.118 Preserve the installed `0.1.5-dev.5` cursor-only screenshot and runtime/database evidence, deterministically reproduce that release tests hid ordinary-launch failure by overriding the 8-second production readiness default to 15 seconds, adopt one production default that passes constrained cold-start simulation, and require affected startup simulations to exercise that default.
- [x] 1.119 Implement simulation-first exact-artifact publication: make the preview workflow pack once, bind machine-readable certification to commit/version/integrity/platform/runtime/default inventory, install and launch that exact tarball under production defaults plus affected neighboring simulations before the complete release gate, reject changed or missing evidence and retry-until-green, then publish a new immutable preview only after installed exact-tarball startup succeeds.
- [x] 1.120 Preserve the installed `0.1.5-dev.6` AddOne-versus-vanilla videos and endpoint failure, deterministically reproduce accumulated exited surfaces exceeding the control-frame bound and Win32 Ctrl+C losing its key identity, bound the initial fullscreen snapshot to the active selected generation, preserve Win32 control-key round trips, avoid disabling caller-owned paste/focus modes before handoff, and pass the focused protocol/input plus containing walking-skeleton gates.
