...
- [ ] 3.1 Run the cleanup command in dry-run mode against the current clone and review the exact protected, merged-deletable, and unmerged sets without altering working-tree files.
- [ ] 3.2 Apply cleanup to the reviewed merged-deletable local topic and milestone branches while retaining `develop`, `master`, and every unmerged branch; verify the resulting branch inventory.
