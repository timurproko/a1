...
- [ ] 4.1 Add deterministic unit/integration coverage for parsing, npm argv, exact target pinning, output capture, progress, activation-event validation, launcher checks, existing-install delegation, failure classification, and cancellation; verify independent negative controls would expose inherited stdio or a moving install tag.
- [ ] 4.2 Exercise the exact installer and A1 tarballs in disposable global prefixes on Windows Node 22/24, Linux Node 24, and macOS Node 24; verify launcher forms, active release, clean first launch, success/failure bytes, and no writes to real user npm/A1 state.
- [ ] 4.3 Add a successful-warning fixture containing representative deprecation, funding, install-script, package-count, and npm-upgrade messages; verify an interactive capture observes only one progress row and `a1 successfully installed`, and redirected output observes only the final success line.
