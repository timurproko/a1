...
- Make every delivery PR start with a quoted `Phase` status, followed by a human-readable `Implementation` summary distilled from the proposal. Advance the phase from `Planning` to `Implementation` to `Acceptance`, omit routine validation command noise, and place required machine linkage last under `Automation` in a clearly explained collapsed disclosure.
