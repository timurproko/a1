...
- Unify every agent-created OpenSpec delivery PR around a quoted `Phase` status, a `Proposal` section containing one or two sentences of intent, an `Implementation` section describing the work, final `Acceptance` outcomes, and collapsed machine linkage under last `Automation`. Advance the visible phase from `Proposal` to `Implementation`, switch to `Acceptance` only after exact-head required product tests pass and the phase gate is the sole expected blocker, and derive `Archived` after verified integration without editing the accepted body; omit routine validation command noise.
