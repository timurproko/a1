...
- Make every draft PR lead with a short human-readable `Intent` summary distilled from the proposal's motivation and main changes. Omit routine validation command noise and place required machine linkage last in a clearly explained collapsed `Automation metadata` section.
