...
The familiar fence avoids a second mutable marker and lets complete base/head lifecycle classification retain the current implementation hold. Every agent-created OpenSpec delivery PR starts with a quoted status line: `> Phase: Proposal` for the initial planning draft, `> Phase: Implementation` from explicit approval through finalization and pending/failed tests, and `> Phase: Acceptance` only after every required exact-head product test passes and the phase gate is the sole expected blocker. The phase-only body edit receives renewed validation before merge. After manual merge, trusted verification derives `Archived` without mutating the accepted PR body. `## Proposal` contains one or two sentences explaining intent, `## Implementation` presents two to five concrete delivery points, `## Acceptance` appears only for the completed candidate, and `## Automation` ends the body with a collapsed disclosure explaining the linkage JSON. Routine validation commands do not obscure these human sections. Version 1 and 2 remain readable and preserve their existing behavior. Unknown versions fail closed.
