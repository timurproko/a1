...
The familiar fence avoids a second mutable marker and lets complete base/head lifecycle classification retain the current implementation hold. The PR body starts with a quoted status line that advances through `> Phase: Planning`, `> Phase: Implementation`, and `> Phase: Acceptance`. `## Implementation` then presents two to five concise proposal-derived points; `## Acceptance` appears only for the completed candidate; and `## Automation` ends the body with a collapsed disclosure explaining the linkage JSON. Routine validation commands do not obscure this summary. Version 1 and 2 remain readable and preserve their existing behavior. Unknown versions fail closed.
