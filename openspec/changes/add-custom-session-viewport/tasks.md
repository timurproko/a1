...
- [ ] 4.1 Declare live A1 settings `scrollbarAppearance` (`always`, `hover`, `hidden`; default `hover`), `scrollbarStyle` (`thin`, `thick`; default `thin`), and `scrollbarSpeed` (`normal`, `fast`; default `normal`) grouped under `Scrollbar`; verify declaration, resolution, section, persistence, and invalid-value tests
- [ ] 4.2 Pass the loaded settings session into the bare-A1 shell and subscribe the viewport to live appearance/style/speed changes; verify normal moves three wheel rows and edge-selects every 30ms, fast doubles both rates, changes require no restart, scroll/follow state is preserved, and only the active profile's A1 settings document is written
