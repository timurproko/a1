...
- [x] 4.1 Declare live A1 settings `scrollbarAppearance` (`always`, `hover`, `hidden`; default `hover`) and `scrollbarStyle` (`thin`, `thick`; default `thin`) with no speed declaration; verify settings declaration, resolution, section, persistence, invalid-value, and no-speed inventory tests
- [x] 4.2 Pass the loaded settings session into the bare-A1 shell through composition and subscribe the viewport to live appearance/style changes; verify integration tests prove a change repaints without restart while preserving scroll/follow state and writes only the active profile's A1 settings document
