...
- [ ] 4.1 Declare live A1 settings `scrollbarAppearance` (`always`, `hover`, `hidden`; default `hover`), `scrollbarStyle` (`thin`, `thick`; default `thin`), and `scrollbarSpeed` (`normal`, `fast`, `high`; default `normal`) grouped under `Scrollbar`; verify declaration, resolution, section, persistence, and invalid-value tests
