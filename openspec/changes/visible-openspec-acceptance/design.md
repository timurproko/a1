...
Store a versioned request at `openspec/acceptance/<change>/<source-head>.json`, outside the active change tree. The dedicated branch is `docs/accept-<change>-<source-pr>` and the title is `#<source-pr>(accept): <original implementation subject>`, stripping only the implementation title's conventional type/scope prefix for readability. The generated PR body is intentionally limited to a link to the original implementation and an unchecked list of the maintainer's required verification items; completed task inventory, hashes, and detailed evidence stay in the committed record and its diff. Labels and editable display metadata help navigation but never establish authority.
...
A request may be created while evidence is missing, and the body lists each unresolved verification item. It stays draft until substantive evidence is complete unless a maintainer deliberately makes it ready for review. Current-head candidate CI verifies schema, scope, immutable source bindings, and referenced-evidence integrity; unresolved review items are reported but do not make an otherwise valid record fail CI. Post-merge receipt consumption still requires complete evidence before archival, so making or merging an incomplete request does not fabricate completion. The accepting maintainer does not need to compose a JSON comment. The agent may prepare evidence-backed record edits in that same PR; automation must preserve those human-reviewed edits rather than regenerating them blindly.
...
Acceptance candidate validation uses trusted repository policy and read-only credentials. It verifies strict schema/size/path bounds, exact single-record diff scope, immutable source bindings, and the integrity of any referenced task evidence. It reports missing implementation CI, pending task verification, and known gaps as review items rather than failing the candidate-integrity check. Complete evidence and final-head implementation CI remain mandatory when a merged request is consumed as an authoritative receipt. The validator must not wait for its own check to become successful: acceptance-head CI is verified later when consuming the merged receipt. Candidate-controlled scripts are never executed with publication credentials. The App creates real PR-triggered CI using existing permissions.
