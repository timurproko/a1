...
// Compatibility: expected decoration permits only menu background and trailing cell padding.
function shadedReference(row: string, width: number): string {
  const background = piTheme().getBgAnsi("toolPendingBg");
  return background + row.replace(/\u001b\[[0-9;:]*m/gu, sequence => sequence + background)
    + " ".repeat(Math.max(0, width - visibleWidth(row))) + "\u001b[49m";
}
...
describe.each([false, true])("above-prompt autocomplete (history=%s)", history => {
  ...
  it("shades every menu cell without a top line or background bleed across themes and resize", async () => {
    const { editor, dispose } = await fixture(history);
    const originalTheme = currentPiThemeName();
    try {
      editor.addAutocompleteProvider(() => ({ ...provider, getSuggestions: async () => ({ prefix: "@", items: items.map(item => ({
        ...item, label: `日本語 é ${item.label}`, description: "before\u001b[0m middle\u001b[49m after",
      })) }) }));
      editor.handleInput?.("@");
      await expect.poll(() => parts(editor, 80).menu.length).toBeGreaterThan(0);
      for (const [theme, color] of [["dark", 0x282832], ["light", 0xe8e8f0]] as const) {
        expect(applyPiTheme(theme, false, "truecolor").success).toBe(true);
        for (const width of [24, 80]) {
          const frame = parts(editor, width);
          expect(frame.menu).toHaveLength(6);
          expect(stripTerminalSequences(frame.menu[0]!)).not.toMatch(/^─/u);
          expect(frame.menu.map(stripTerminalSequences).join("\n")).toContain("→");
          for (const row of frame.menu) expect(visibleWidth(row)).toBe(width);
          const data = frame.rows.join("\r\n");
          const cells = await replayTerminalBackgroundCells([{ data, atMs: 0 }], { columns: width, rows: frame.rows.length });
          expect(cells).toHaveLength(width * frame.menu.length);
          expect(cells.every(cell => cell.row <= frame.menu.length && cell.color === color)).toBe(true);
          editor.handleInput?.("\u001b[B");
          expect(parts(editor, width).geometry).toEqual(frame.geometry);
        }
      }
      editor.handleInput?.("\u001b");
      expect(parts(editor, 80).geometry.rowOffset).toBe(0);
      const rows = editor.render(80);
      expect(await replayTerminalBackgroundCells([{ data: rows.join("\r\n"), atMs: 0 }], { columns: 80, rows: rows.length })).toEqual([]);
      ...
      for (const limit of [3, 5, 12, 20]) {
        ...
        for (let selected = 0; selected < 14; selected++) {
          for (const width of [12, 40, 80]) {
            ...
            expect(actual.menu).toEqual(expected.map(row => shadedReference(row, width)));
          }
          for (const target of [editor, reference.editor]) {
        ...
        expect(parts(editor, 80).menu).toEqual(reference.editor.render(78).slice(3).map(row => shadedReference(`  ${row}`, 80)));
