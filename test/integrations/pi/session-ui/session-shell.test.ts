  ...
  it.each([false, true].flatMap(history => ["empty", "long", "detached", "streaming"].map(state => ({ history, state }))))(
    "paints stable autocomplete checkpoints ($state, history=$history)", async ({ history, state }) => {
      ...
      try {
        ...
        const expected: Array<{ rows: string[]; rawRows: readonly string[]; cursorRow: number }> = [];
        const capture = async () => {
          ...
          expected.push({ rawRows: frame, rows: frame.map(row => stripTerminalSequences(row).trimEnd()),
        ...
        for (const [index, frame] of painted.entries()) {
          ...
          if (state === "empty" && checkpoints[index]!.columns === 80) {
            const checkpoint = checkpoints[index]!;
            const actualCells = await replayTerminalBackgroundCells(writes.slice(0, checkpoint.writeEnd), checkpoint);
            const expectedCells = await replayTerminalBackgroundCells([
              { data: expected[index]!.rawRows.join("\r\n"), atMs: 0 },
            ], checkpoint);
            expect(actualCells, `background checkpoint ${index}`).toEqual(expectedCells);
          }
        }
      } finally { await shell.dispose(); }
    });
