...
async function capture(producer: "pinned" | "owned", home: string, modes: readonly string[], cases = COMMAND_MESSAGE_CASES): Promise<readonly Capture[]> {
  return phases.run(`${producer}-${modes.join("-")}`, async () => {
    ...
    const { stdout, stderr } = await execute(process.execPath, [
      "--import", "tsx", resolve("test/integrations/pi/session-ui/command-message-worker.mjs"), producer, home, JSON.stringify(modes), JSON.stringify(cases), join(tmpdir(), "command-message-logical-agent"),
    ], {
