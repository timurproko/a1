...
const image = new URL("../../../src/app/session-shell/image-worker.ts", import.meta.url);
const client = new URL("../../../src/app/session-shell/image-preparation-client.ts", import.meta.url);
...
describe("cold emitted clipboard worker fixture", () => {
  ...
  it("changes only the exact source bootstrap entry and eval flag", () => {
    ...
    expect(selected.entry).toEqual(new URL("../../../dist/app/session-shell/image-worker.js", import.meta.url));
