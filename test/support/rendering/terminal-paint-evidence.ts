...
export async function replayTerminalCheckpoints(
  writes: readonly TimedTerminalWrite[],
  checkpoints: readonly TerminalReplayCheckpoint[],
): Promise<readonly TerminalCellFrame[]> {
  const first = checkpoints[0];
  if (first === undefined) return [];
  const terminal = new Terminal({ cols: first.columns, rows: first.rows, allowProposedApi: true, scrollback: 0 });
  const frames: TerminalCellFrame[] = [];
  let writeStart = 0;
  try {
    for (const [index, checkpoint] of checkpoints.entries()) {
      if (!Number.isSafeInteger(checkpoint.writeEnd) || checkpoint.writeEnd < writeStart || checkpoint.writeEnd > writes.length) {
        throw new RangeError(`terminal replay checkpoint ${index} has an invalid write boundary`);
      }
      if (!Number.isSafeInteger(checkpoint.columns) || checkpoint.columns < 1
        || !Number.isSafeInteger(checkpoint.rows) || checkpoint.rows < 1) {
        throw new RangeError(`terminal replay checkpoint ${index} has invalid geometry`);
      }
      if (terminal.cols !== checkpoint.columns || terminal.rows !== checkpoint.rows) {
        terminal.resize(checkpoint.columns, checkpoint.rows);
      }
      for (const write of writes.slice(writeStart, checkpoint.writeEnd)) await terminalWrite(terminal, write.data);
...
export async function replayTerminalPaint(
  ...
  // Invariant: classification validates synchronized-output structure before replaying bytes.
...
function hasRowPaintAfter(data: string, from: number): boolean {
  ...
  // Invariant: a cursor move that positions a regional S/T operation is movement metadata,
  // not an addressed row repaint.
  if (/^\u001b\[(?:\d+)?[ST]/u.test(segment)) return false;
