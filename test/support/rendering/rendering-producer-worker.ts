...
async function runOwned(
  ...
  try {
    ...
    checkpoints.push(ownedCheckpoint("initial", 0, terminal, shell.view().transcript));
    for (const step of steps) {
      ...
      await adapter.flushEvents();
      shell.runtime.renderNow();
      checkpoints.push(ownedCheckpoint(step.checkpoint, step.atMs, terminal, shell.view().transcript));
    }
    return {
...
class PinnedRoot implements Component {
  ...
  applyEvent(event: Readonly<Record<string, unknown>>): void {
    if (event.type === "agent_start") this.#working = true;
    else if (event.type === "agent_settled" || event.type === "agent_end") this.#working = false;
      ...
      if (isRecord(message) && message.role === "user" && event.type === "message_start") {
        ...
        if (this.#assistant === undefined) {
          ...
          this.transcript.push({ kind: contentKind(message.content), status, text });
        } else this.transcript[this.#assistantIndex] = { kind: contentKind(message.content), status, text };
...
function pinnedCheckpoint(
  ...
  return ownedCheckpoint(name, atMs, terminal, transcript);
}
...
function textFromContent(value: unknown): string {
  ...
  return value.map(item => isRecord(item)
    ? typeof item.text === "string" ? item.text : typeof item.thinking === "string" ? item.thinking : ""
    : "").join("");
}

function contentKind(value: unknown): string {
  return Array.isArray(value) && value.some(item => isRecord(item) && item.type === "thinking") ? "thinking" : "assistant";
}

async function main(): Promise<void> {
  const request = await readRequest();
  if (request.testBehavior === "hang") await new Promise(() => {});
