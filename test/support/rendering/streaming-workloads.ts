...
export const STREAM_RENDERING_WORKLOADS: readonly RenderingWorkload[] = Object.freeze([
  ...
  },
  {
    ...
    rows: 14,
    steps: [
      ...assistantStream("resize", ["before", "before resize"], 0),
      { checkpoint: "resize-structural", atMs: 15, action: { type: "resize", columns: 38, rows: 10 } },
      event("resize-after", 20, {
        type: "message_update",
  ...
  },
  {
    ...
    rows: 12,
    steps: [
      ...assistantStream("detached", proseValues, 0),
      { checkpoint: "detached-wheel-up", atMs: 30, action: { type: "input", data: "\u001b[<64;5;4M" } },
      event("detached-new-output", 35, {
        type: "message_update",
