...
describe("owned UI launch documentation", () => {
  it("keeps bare, comparison, sandbox, and removed-ui guidance consistent with CLI dispatch", async () => {
    ...
    expect(checkpoint).toContain("All three commands use the same A1-owned rendering and input pipeline");
    expect(checkpoint).toContain("For recovery, use `a1 pi`");
    expect(cliUsage(prerelease)).toBe(
      "Usage: a1 | a1 pi | a1 sandbox | a1 version | a1 update [self|--models]"
