...
function assertDevelopmentStartup(workflow: ReturnType<typeof parse>) {
  ...
  expect(startup.steps).toEqual(expect.arrayContaining([
    ...
    expect.objectContaining({ name: "Validate first-attempt exact-package startup budget", run: "node scripts/release/run-validation-tier.mjs package-startup --result .artifacts/validation/startup-node${{ matrix.node }}.json", env: expect.objectContaining({ STARTUP_PERFORMANCE_RESULT: ".artifacts/validation/startup-node${{ matrix.node }}-performance.json" }) }),
    expect.objectContaining({ name: "Validate exact-package identity, layers, recovery, and cleanup", run: "node scripts/release/run-validation-tier.mjs package-contracts --result .artifacts/validation/package-contracts-node${{ matrix.node }}.json", env: expect.objectContaining({ VALIDATION_BUILD_READY: "1", VALIDATION_CANDIDATE_TARBALL: "${{ github.workspace }}/.artifacts/validation/package/candidate.tgz" }) }),
