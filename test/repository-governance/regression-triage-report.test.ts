...
describe("nightly regression triage report", () => {
  ...
  it("triages failed Full regression runs and scheduled Release runs only", () => {
    expect(triageDecision({ id: 1, workflowName: "Full regression", conclusion: "failure", event: "workflow_dispatch" })).toMatchObject({ triage: true, workflow: { file: "full-regression.yml" } });
    expect(triageDecision({ id: 1, workflowName: "Release", conclusion: "failure", event: "schedule" })).toMatchObject({ triage: true, workflow: { file: "release.yml", scheduledOnly: true } });
    expect(triageDecision({ id: 1, workflowName: "Release", conclusion: "failure", event: "workflow_dispatch" })).toMatchObject({ triage: false, reason: expect.stringContaining("scheduled runs only") });
    expect(triageDecision({ id: 1, workflowName: "Full regression", conclusion: "cancelled", event: "schedule" })).toMatchObject({ triage: false, reason: "run 1 concluded cancelled, not failure" });
    expect(triageDecision({ id: 1, workflowName: "Development validation", conclusion: "failure", event: "pull_request" })).toMatchObject({ triage: false });
  });
