...
describe("docs-sensitive governance", () => {
  it("runs lightweight inventory consistency for every docs-only pull request", async () => {
    ...
    expect(docsJob).toContain("if: needs.changes.outputs.acceptance-only != 'true' && needs.changes.outputs.acceptance-phase != 'true' && needs.changes.outputs.docs-only == 'true'");
