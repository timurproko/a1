...
describe("version-3 GitHub delivery authority", () => {
  ...
  it("rejects the superseded phase-prefixed layout for an open candidate", async () => {
    ...
    await expect(validateVersion3Candidate(f.reader, 42)).rejects.toThrow("acceptance-layout-phase");
  });

  it("retains read-only compatibility for an immutable merged phase-prefixed delivery", async () => {
    const f = fixture(true);
    f.pull.body = `> Phase: Implementation\n\n${f.pull.body}`;
    await expect(loadArchiveEvidence(f.reader, 42)).resolves.toMatchObject({
      disposition: "eligible", implementation: { version: 3 }, acceptance: { kind: "single-pr", checks: scenarios },
    });
  });
