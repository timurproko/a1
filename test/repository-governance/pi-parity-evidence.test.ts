...
describe("Pi parity machine-readable evidence", () => {
  it("binds passed parity to exact packages, fixtures, tolerances, and gates", async () => {
    const evidence = JSON.parse(await readFile(
      ...
      fixtures: { path: string; sha256: string; coverage: string[]; tolerance: { ignored: string[]; preserved: string[] } }[];
    ...
    expect(evidence.manualAcceptanceStatus).toBe("pending");
    ...
    expect(evidence.visualDivergences).toEqual([]);
