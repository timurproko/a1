...
The central disposable policy is `node_modules`, `dist`, `.builds`, `native/process-guardian/target`, `.artifacts/openspec-archive`, and `.artifacts/validation`. The native root contains repository-generated process-guardian Cargo output, and the last root contains repository-generated validation selection and result reports. Each encountered path must be ignored and stay inside the exact worktree with no link, special file, or nested repository boundary. Authority is component-exact: another `target` directory, `native/process-guardian`, `native/terminal-host/target`, `native/process-guardian/target-user`, `.artifacts`, sibling directories such as `.artifacts/other`, and near matches such as `.artifacts/validation-user` remain blocking. Tracked/staged/unstaged/untracked content and every unknown ignored path still block. A tracked regular `.gitmodules` file alone is ordinary content; actual nested `.git` metadata, gitlinks, configured submodules, and submodule changes block. Ordinary content and these approved generated roots are traversed under separate finite entry allowances, so normal dependency and Cargo build trees do not consume the ordinary source-tree allowance; both allowances retain the same deadline and content-boundary checks.
