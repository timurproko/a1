...
| --- | --- |
| Pull request into `develop` | Bounded PR-cadence validation; changed/new source documentation is checked once, rendering runs as `none`, `smoke`, or `full`, exhaustive owners are reported as deferred, and selected repair/publishing-impact changes add PR-attached complete regression |
...
`Development validation required` remains the merge gate for every pull request. A trusted base-controlled selector binds current PR metadata, merge base, exact head, target, paths (including renamed-from and deleted repair history), and selection identity. When selected, the same PR run calls `.github/workflows/full-regression-shared.yml` and requires Windows 2025 Node 22/24, Ubuntu 24.04 Node 24, and macOS 15 Node 24 plus their exact-run outcome envelopes. Failed, cancelled, missing, stale, mismatched, or unexpectedly skipped work blocks the stable aggregate. Standalone `.github/workflows/full-regression.yml` retains its name, schedule, dispatch, non-cancelling behavior, and non-publishing authority; its result cannot substitute for selected PR evidence.
...
Record the run's resolved source SHA and package evidence; do not substitute an unrelated historical green run for current-head acceptance. This standalone run is a deliberate additional operation, not an automatic step for every PR and not selected PR authority.
...
- **Nightly regression fails:** the triage workflow opens or refreshes a draft `fix/nightly-regression-<date>` pull request whose body and change carry the evidence; start the fix there. Reproduce from the listed tests or commands on the failed lane and fix without weakening assertions, budgets, timeouts, or coverage. Record focused implementation evidence and pre-finalization observations in the change, then rely on automatically selected PR-attached Full regression and `Development validation required` on the finalized exact head. Put its final run/head/selection in Actions and handoff rather than making another evidence-recording commit. Standalone dispatch remains diagnostic and pre-deployment obligations remain in force; numbered-package nightly recovery is independent. A day whose failed scope set differs from every open candidate opens a new one. A Full regression dispatched on a candidate's own branch is that candidate's proof but never selected PR authority; its failures never open another candidate. A failure with no candidate means the triage workflow itself failed; read its run summary and dispatch it with the failed run id and `dry_run` off.
