...
`node scripts/pi/propose-pi-upgrade.mjs [--version <v>] [--commit <sha>] [--output <dir>]` proposes a Pi upgrade in the working tree: it captures the old upstream sources, pins both packages at the target (the newest published version by default), evaluates the candidate through `pi-candidate-evaluator.mjs`, installs and builds, three-way merges each vendored copy under `src/integrations/pi/components/upstream/` (`git merge-file --diff3` of the A1 copy, the old upstream, and the new upstream; conflict markers stay in the file as review items), regenerates the ledger and headers, re-resolves the inventories, refreshes parity evidence, runs the type, architecture, engine-conformance, and parity gates, scaffolds `openspec/changes/pi-upgrade-<version>/` with the pin moved in the requirement that names it, and writes `report.json` and the pull-request `body.md`. A failed step is recorded and the run continues. `.github/workflows/pi-upstream-sync.yml` runs it nightly and opens or refreshes the draft pull request `chore/pi-<version>` with the App identity; a human resolves the review items and merges. The pinned identity every check compares itself to comes from `scripts/governance/pinned-pi-identity.mjs`: versions from the dependency authority, the commit from `pinned-pi-interactive-baseline.json`.
