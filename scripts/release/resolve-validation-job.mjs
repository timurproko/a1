...
const result = { schema: "a1-validation-job-selection-v3", head: impact.head, selectionId: impact.selectionId, job, platform, architecture, node, active, owners: ownerIds, deferredOwners, scopes, tests };
process.stdout.write(`${JSON.stringify(result)}\n`);
if (process.env.GITHUB_OUTPUT) {
  const { appendFile } = await import("node:fs/promises");
  await appendFile(process.env.GITHUB_OUTPUT, `active=${active}\nhead=${impact.head}\nselection_id=${impact.selectionId}\nowners_json=${JSON.stringify(ownerIds)}\ndeferred_owners_json=${JSON.stringify(deferredOwners)}\nscopes_json=${JSON.stringify(scopes)}\ntests_json=${JSON.stringify(tests)}\n`);
}
