...
Commands: preview (default), status, enable, disable, once, watch, register, claim, release, recover
Registration: --path PATH --change NAME --source-pr N --candidate-pr N --role implementation|archive|acceptance
Optional registration: --disposable node_modules (repeat for each explicitly disposable generated path)
...
export async function main(args = process.argv.slice(2)) {
  const { values, positionals } = parseArgs({ args, allowPositionals: true, strict: true, options: {
    repo: { type: "string" }, path: { type: "string" }, change: { type: "string" }, "source-pr": { type: "string" }, "candidate-pr": { type: "string" },
  ...
  if (!["preview", "status", "enable", "disable", "once", "watch", "register", "claim", "release", "recover"].includes(command)) fail("unknown-command");
  ...
  if (["once", "watch"].includes(command) && inside(identity.root, await canonical(fileURLToPath(import.meta.url)))) fail("worker-code-inside-removable-root");
  let cancelled = false, nextAllowed = 0;
  ...
  try {
    async function run() {
      ...
      if (!token) {
        ...
        catch { /* Security: unauthenticated preview can still report local unmanaged paths; private evidence will fail closed. */ }
      }
      const reader = cleanupReader({ repository: identity.repository, token, deadline, onBackoff: time => { nextAllowed = Math.max(nextAllowed, time); } });
