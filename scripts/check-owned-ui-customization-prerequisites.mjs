...
for (const file of await walk(resolve(root, "src"))) {
  ...
  const debtScope = /^(?:src\/features\/owned-ui|src\/foundation\/(?:pi-engine-adapter|pi-component-adapter|pi-tui-runtime-adapter))\//.test(path);
...
const ledgerPath = resolve(root, "openspec", "changes", "build-owned-pi-ui-foundation", "evidence", "pinned-pi-source-port-ledger.json");
try {
