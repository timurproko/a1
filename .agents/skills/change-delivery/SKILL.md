...
6. Keep the finalized PR at `Implementation` and mark it ready for exact-head CI. Once every required product test passes and only the phase gate blocks, change only the first line to `> Phase: Acceptance`; require body-edit validation before handoff. A new commit, acceptance list, or target requires revalidation.
