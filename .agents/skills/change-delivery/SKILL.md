...
6. One exact-head CI run validates the finalized delivery and selected scopes. When the stable protected aggregate succeeds, hand off the unchanged PR for manual merge, then run `local-worktree-cleanup.mjs handoff` from primary for the exact worktree, change, and PR (again after repairs). Do not add a lifecycle body edit, start a second validation run, wait for merge, or start a watcher; a new commit, acceptance list, or target requires revalidation.
7. Nightly-triage `fix/nightly-regression-<date>` candidate: before hand-off, dispatch `gh workflow run full-regression.yml --ref <branch>` on the completed fix head and record the passing run in `design.md` before hand-off.
