...
7. After the implementation merges, trusted automation prepares or reuses its visible acceptance-record PR. It must show actual exact-head CI/evidence and every pending task; reconcile only evidence-backed stale bookkeeping in that same PR. Do not create another proposal. Its current-head CI uses trusted base policy, and it must be merged manually by an authorized maintainer—never auto-merge it. That verified merge supplies archival acceptance without a JSON comment or recursive acceptance request. Legacy valid comment acceptance remains readable.
