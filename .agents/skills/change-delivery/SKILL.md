...
6. After applicable CI passes, hand off for actual maintainer validation using the project manual-test command format. Ready status is not acceptance or merge authority. Merge implementation manually only with explicit authorization. Never enable code or implementation-bound auto-merge. New candidate commits require current-head CI and renewed review; never fabricate review or auto-check substantive work.
7. After the implementation merges, trusted automation prepares or reuses its visible acceptance-record PR titled `#<source PR>(accept): <original implementation subject>`. Keep its body to the original PR link and the maintainer's required verification checklist; exact identities and detailed evidence remain in the committed record. Reconcile only evidence-backed stale bookkeeping in that same PR. Do not create another proposal. Current-head candidate CI uses trusted base policy to validate integrity and may report unchecked review items without failing; post-merge receipt consumption still requires complete evidence. An authorized maintainer must merge it manually—never auto-merge it. Only a verified complete merge supplies archival acceptance without a JSON comment or recursive acceptance request. Legacy valid comment acceptance remains readable.
8. Let the archive workflow synchronize canonical specs and prepare the OpenSpec-only archive PR only after verified acceptance. That follow-up uses documentation auto-merge after its own required CI. Only the two exact optional mechanical archive tasks may remain for their corresponding verified operations. Confirm archive **integration**, not just creation, before eligible owning-agent cleanup of retained task/acceptance worktrees under the existing ownership and cleanliness checks. Do not remove another session's worktree or treat remote-branch deletion as proof that local cleanup is safe.
...
The documentation merge owner holds drafts, explicit implementation links, newly introduced active change directories, and authoritative acceptance records/branches even when editable markers are removed or renamed. It disables any armed acceptance auto-merge; verified archive copies remain eligible. Do not bypass the hold. After an eligible accepted manual merge, the archive workflow handles synchronization and its separate CI-gated archive PR. Missing evidence, unsupported inputs, or App setup remain blockers; live activation/provisioning require separate authorization.
