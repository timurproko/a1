...
The non-interactive gate runs Rust layout/topology tests, retained-model/one-PTY, selection, native key/text/mouse/clipboard, and four-PTY integration probes. The 2×2 probe verifies durable pane/session mappings, exact argv, per-pane environment and cwd, distinct process identities, focused-input isolation, all-pane resize, and cleanup. It also emits bounded `addone-terminal-host-hot-path-v1` metadata proving four distinct native stream, input, terminal-model, render-damage, key-encoder, mouse-encoder, and selection identities. Raw PTY bytes, child input, and rendered cells remain in Rust and are not exported in the metadata. The gate does not enter alternate-screen mode.
