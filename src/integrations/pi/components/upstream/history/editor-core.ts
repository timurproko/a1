/**
 * Provenance: @earendil-works/pi-tui 0.87.0 (MIT), commit 16787ad5b2dc748047f314ca1bfe7708f30f54f3,
...
export class HistoryEditorCore implements Component, Focusable {
	private segmentTransform: Parameters<EditorInteractionPort["setSegmentTransform"]>[0] | undefined;
	...
	readonly recall: EditorRecallPort = {
		...
		observe: listener => {
			...
			return () => { if (this.onHistoryChange === listener) delete this.onHistoryChange; };
		},
	...
	replaceHistoryEntries(entries: readonly string[]): void {
		...
		if (this.historyIndex >= 0) this.pendingHistory = snapshot;
		else this.history = snapshot;
	}

	getHistoryPosition(): { readonly index: number; readonly total: number } {
	...
	private applyPendingHistory(): void {
		if (this.pendingHistory === undefined) return;
	...
	private navigateHistory(direction: 1 | -1): void {
		...
		if (this.historyIndex === -1 && newIndex >= 0) {
			...
			if (this.persistentHistory) this.historyPasteDraft = { pastes: this.pastes, counter: this.pasteCounter };
		}
		...
		if (this.historyIndex === -1) this.applyPendingHistory();
		this.notifyHistory();
	}
	...
	private exitHistoryBrowsing(): void {
		...
		if (wasBrowsing) this.notifyHistory();
	}
