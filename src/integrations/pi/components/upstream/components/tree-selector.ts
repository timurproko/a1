...
function formatKeyText(key: string): string {
	return key
		.split("/")
		.map((value) => value.split("+").map((part) => process.platform === "darwin" && part.toLowerCase() === "alt" ? "option" : part).join("+"))
		.join("/");
}
	...
	updateNodeLabel(entryId: string, label: string | undefined, labelTimestamp?: string): void {
		for (const flatNode of this.flatNodes) {
			if (flatNode.node.entry.id === entryId) {
				if (label === undefined) {
					delete flatNode.node.label;
					delete flatNode.node.labelTimestamp;
				} else {
					flatNode.node.label = label;
					flatNode.node.labelTimestamp = labelTimestamp ?? new Date().toISOString();
				}
				break;
			}
		}
...
function formatHelpKeys(keybindings: Keybinding[]): string {
	...
	for (const keybinding of keybindings) {
		const key = treeKeybindings.getKeys(keybinding)[0];
