...
export function createPiShellEditor(options: PiShellEditorOptions): PiShellEditorPort {
  ...
  return {
    ...
    render: width => {
      ...
      bodyGeometry = { rowOffset: rows.length - rowCount, rowCount };
      ...
      return [...rows.slice(rowCount).map(row => paintAutocompleteRow(row, width)), ...rows.slice(0, rowCount)];
    },
